...
/**
 * Class RadioStationProfileStreamDatatable
 *
 * @package AppBundle\Datatables
 */
class RadioStationProfileStreamDatatable extends AbstractDatatableView
{

    public function getContenType()
    {
        return $this->em->getRepository('AppBundle:ContentType')->findAll();
    }
    ...
    public function getLineFormatter()
    {
        ...
        $formatter = function ($line) {
            if($line['slogan']){
                ...
                if (strlen($line['title']) > 30) {
                    $line['title'] = $this->truncate($line['title'], 30);
                }
            }
            ...
            if (isset($line['contentType'])) {
                $line['contentType']['title'] = ucfirst($line['contentType']['title']);
            }
    ...
    public function buildDatatable(array $options = array())
    {
        ...
        if(isset($options['radioStationId'])){
            $this->ajax->set(array(
                'url' => $this->router->generate('radio_station_profile_results',array(
            ...
            $this->ajax->set(array(
                'url' => $this->router->generate('radio_station_profile_results'),
                'type' => 'GET',
                'pipeline' => 0
            ));
        }
